...
class Generator(Dataset):
  ...
  def __init__(
    ...
    if type_input == 'image':
      ...
      if self.normalize_head == 'mean_std':
        self.transform_head = lambda x: torch.tensor((x - self.values_normalize_head[0:1]) / self.values_normalize_head[1:2])
      else:
    ...
    if type_output == 'image':
      ...
      if self.normalize_tail == 'mean_std':
        self.transform_tail = lambda x: torch.tensor((x - self.values_normalize_tail[0:1]) / self.values_normalize_tail[1:2])
      else:
  ...
  def undo_normalize(self, x, type):
    if type == 'head':
      if self.normalize_head == 'mean_std':
        return x * self.values_normalize_head[1:2] + self.values_normalize_head[0:1]
      else:
        return x
    elif type == 'tail':
      if self.normalize_tail == 'mean_std':
        return x * self.values_normalize_tail[1:2] + self.values_normalize_tail[0:1]
      else:
        return x
    else:
      ...
      if self.dataset == 'train':
        if os.path.exists(name_file):
          ...
          if norm_by_channel:
            ...
            init             = np.zeros_like(np.mean(x, axis=(0, 1)), dtype=np.float32)
            values_normalize = np.array([init, init])
            for path in tqdm(self.partition, desc=f'{normalize}_{name}'):
              x = self.process_path(path)[index]
              if normalize == 'mean_std':
                values_normalize[0] += np.mean(x, axis=(0, 1))
                values_normalize[1] += np.std(x, axis=(0, 1))
          else:
          ...
          values_normalize = np.reshape(values_normalize, [2, 1, 16, 1])
