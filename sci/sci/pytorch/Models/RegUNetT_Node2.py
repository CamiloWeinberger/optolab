...
class RegUNetT_Node2(ModelBase, pl.LightningModule):
  def __init__(self):
    super().__init__()
    ...
    self.node = AnyStage(64, 32, 2, 2, ResBottleneckBlock_Reversed, 1, 16, True, False, True)
    self.revsci = nn.Sequential(
  ...
  def forward(self, x):
    ...
    revsci = self.revsci(node)
    range = self.range(torch.tanh(revsci))
