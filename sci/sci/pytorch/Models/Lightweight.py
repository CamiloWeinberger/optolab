...
class Lightweight(ModelBase, pl.LightningModule):
  def __init__(self,):
    ...
    # self.metrics['sam']           = SpectralAngleMapper()
    # self.metrics['sdi']           = SpectralDistortionIndex()
    # self.metrics['uiqi']          = UniversalImageQualityIndex(data_range=255)
    # self.metrics['mape']          = lambda x, y: torch.mean(torch.abs((x - y) / y))
    # self.metrics['accuracy']      = lambda x, y: accuracy(x, y, topk=(1,))
    # self.metrics['accuracy_top5'] = lambda x, y: accuracy(x, y, topk=(5,))

    # self.metrics['dice']          = DiceLoss()
    # self.metrics['dice_bce']      = DiceBCELoss()
    # self.metrics['iou']           = IoULoss()
    # self.metrics['focal']         = FocalLoss()
    # self.metrics['tversky']       = TverskyLoss()
    # self.metrics['focal_tversky'] = FocalTverskyLoss()
    # self.metrics['combo']         = ComboLoss()
    # self.metrics['lovasz_hinge']  = LovaszHingeLoss()
    # self.metrics['cross_entropy'] = CrossEntropyLoss()
    # self.metrics['bce']           = BCELoss()
    # self.metrics['bce_with_logits'] = BCEWithLogitsLoss()
