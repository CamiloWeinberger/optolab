...
def window_partition(x, window_size):
    ...
    windows = x.permute(0, 1, 2, 4, 3, 5, 6).contiguous().view(-1, 16, window_size, window_size, C)
    return windows
...
def window_reverse(windows, window_size, H, W):
    ...
    x = windows.view(B, 16, H // window_size, W // window_size, window_size, window_size, -1)
    x = x.permute(0, 1, 2, 4, 3, 5, 6).contiguous().view(B, 16, H, W, -1)
    return x
...
class SE(nn.Module):
    ...
    def __init__(self,
        ...
        super().__init__()
        self.avg_pool = nn.AdaptiveMaxPool3d([16, 1, 1])
        self.fc = nn.Sequential(
            nn.Linear(oup*16, int(inp*16 * expansion), bias=False),
            nn.GELU(),
            nn.Linear(int(inp*16 * expansion), oup*16, bias=False),
            nn.Sigmoid()
        )

    def forward(self, x):
        b, c, s, _, _ = x.size()
        y = self.avg_pool(x).view(b, c*s)
        y = self.fc(y).view(b, c, s, 1, 1)
        return x * y
...
class PatchEmbed(nn.Module):
    ...
    Global Context Vision Transformers <https://arxiv.org/abs/2206.09959>"
    """
    def __init__(self, in_chans=3, dim=96, use_skip=False, use_revsci=False):
        """
        Args:
...
class WindowAttention(nn.Module):
    ...
    def forward(self, x, q_global):
        B_, S, N, C = x.shape
        qkv = self.qkv(x).reshape(B_, S, N, 3, self.num_heads, C // self.num_heads).permute(3, 0, 1, 4, 2, 5)
        ...
        x = (attn @ v).transpose(1, 2).reshape(B_, S, N, C)
        x = self.proj(x)
        x = self.proj_drop(x)
        return x
...
class WindowAttentionGlobal(nn.Module):
    ...
    def forward(self, x, q_global):
        B_, S, N, C = x.shape
        B = q_global.shape[0]
        kv = self.qkv(x).reshape(B_, S, N, 2, self.num_heads, C // self.num_heads).permute(3, 0, 1, 4, 2, 5)
        k, v = kv[0], kv[1]
        q_global = q_global.repeat(1, B_ // B, 1, 1, 1)
        q = q_global.reshape(B_, 16, self.num_heads, N, C // self.num_heads)
        q = q * self.scale
        attn = (q @ k.transpose(-2, -1))
        relative_position_bias = self.relative_position_bias_table[self.relative_position_index.view(-1)].view(
            self.window_size[0] * self.window_size[1], self.window_size[0] * self.window_size[1], -1)
...
class GCViTBlock(nn.Module):
    ...
    def forward(self, x, q_global):
        ...
        x_windows = x_windows.view(-1, 16, self.window_size * self.window_size, C)
...
class GlobalQueryGen(nn.Module):
    ...
    def __init__(self,
        ...
        if input_resolution == resolution//4:
        ...
        elif input_resolution == resolution//8:
            self.to_q_global = nn.Sequential(
                FeatExtract(dim, keep_dim=False),
                FeatExtract(dim, keep_dim=False),
            )

        elif input_resolution == resolution//16:
        ...
        elif input_resolution == resolution//32:
            self.to_q_global = nn.Sequential(
                FeatExtract(dim, keep_dim=True)
            )
    ...
    def forward(self, x):
        ...
        B = x.shape[0]
        x = x.reshape(B, 16, self.N, self.num_heads, self.dim_head).permute(0, 1, 3, 2, 4)
        return x
...
class GCViTLayer(nn.Module):
    ...
    def __init__(self,
        ...
        self.downsample = None if not downsample else ReduceSize(dim=dim)
        self.q_global_gen = GlobalQueryGen(dim, input_resolution, window_size, num_heads, resolution)
...
class GCViT(nn.Module):
    ...
    def __init__(self,
        ...
        if self.use_head:
            self.avgpool = nn.AdaptiveMaxPool3d(1)
...
if __name__ == '__main__':
    model = gc_vit_xxtiny(pretrained=False)
