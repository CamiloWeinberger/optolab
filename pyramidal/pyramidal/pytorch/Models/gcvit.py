...
class GCViTBlock(nn.Module):
    ...
    def forward(self, x, q_global):
            ...
            x_windows = x_windows.view(-1, self.window_size * self.window_size, torch.tensor(C))
            attn_windows = self.attn(x_windows, q_global)
            x = window_reverse(attn_windows, self.window_size, torch.tensor(H), torch.tensor(W))
...
class GlobalQueryGen(nn.Module):
    ...
    def __init__(self,
        ...
        elif input_resolution == size // 8:
            self.to_q_global = nn.Sequential(
                FeatExtract(dim, keep_dim=False),
                FeatExtract(dim, keep_dim=False),
            )

        elif input_resolution == size // 16:

            if window_size == window_size_orig * 2:
                self.to_q_global = nn.Sequential(
                    FeatExtract(dim, keep_dim=True)
                )

            elif window_size == window_size_orig:
                self.to_q_global = nn.Sequential(
                    FeatExtract(dim, keep_dim=False)
                )

        elif input_resolution == size // 32:
            self.to_q_global = nn.Sequential(
                FeatExtract(dim, keep_dim=True)
            )
...
class GCViTLayer(nn.Module):
    ...
    def __init__(self,
        ...
        self.q_global_gen = GlobalQueryGen(dim, input_resolution, window_size, num_heads, size=size_img, window_size_orig=window_size_orig)
    ...
    def __init__(self,
        ...
        for i in range(len(depths)):
            level = GCViTLayer(dim=int(dim * 2 ** i),
                               ...
                               size_img=resolution,
                               window_size_orig=window_size[0])
...
@register_model
def gc_vit_minimal_half_optolab(pretrained=False, **kwargs):
    model = GCViT(depths=[2, 2, 2],
                  num_heads=[1, 1, 1],
                  window_size=[kwargs['window_size_target'], kwargs['window_size_target'], kwargs['window_size_target']],
                  dim=64,
                  mlp_ratio=1.,
                  drop_path_rate=0.15,
                  **kwargs)
...
@register_model
def gc_vit_minimal_optolab(pretrained=False, **kwargs):
    model = GCViT(depths=[2, 2, 2, 2],
                  num_heads=[1, 1, 1, 1],
                  window_size=[kwargs['window_size_target'], kwargs['window_size_target'], kwargs['window_size_target'], kwargs['window_size_target']],
                  dim=64,
                  mlp_ratio=1.,
                  drop_path_rate=0.15,
                  **kwargs)
...
@register_model
def gc_vit_xtiny_optolab(pretrained=False, **kwargs):
    model = GCViT(depths=[2, 2, 2, 2],
                  num_heads=[2, 2, 2, 2],
                  window_size=[kwargs['window_size_target'], kwargs['window_size_target'], kwargs['window_size_target'], kwargs['window_size_target']],
                  dim=64,
...
@register_model
def gc_vit_tiny_optolab(pretrained=False, **kwargs):
...
def gc_vit_xxtiny(pretrained=False, **kwargs):
    model = GCViT(depths=[2, 2, 6, 2],
                  num_heads=[2, 4, 8, 16],
                  window_size=[kwargs['window_size_target'], kwargs['window_size_target'], kwargs['window_size_target'] * 2, kwargs['window_size_target']],
                  dim=64,
                  mlp_ratio=3,
                  drop_path_rate=0.2,
                  **kwargs)
...
class gcvit(ModelBase, LightningModule):
  def __init__(self, **kwargs):
    ...
    num_classes = int(kwargs['datavariant'])
    self.resize_value = kwargs['resize_value']

    assert kwargs['model_name'] in ['gc_vit_minimal_half_optolab', 'gc_vit_minimal_optolab', 'gc_vit_xtiny_optolab', 'gc_vit_tiny_optolab', 'gc_vit_xxtiny', 'gc_vit_xtiny', 'gc_vit_tiny', 'gc_vit_small', 'gc_vit_base']
    assert float(kwargs['resize_value'] // 32) == kwargs['resize_value'] / 32
    self.model = create_model(kwargs['model_name'],
                              pretrained=False,
                              num_classes=num_classes,
                              multiply_filters=kwargs['multiply_filters'],
                              resolution=self.resize_value,
                              window_size_target=kwargs['resize_value'] // 32)

  def forward(self, x):
    x = F.interpolate(x.unsqueeze(1), size=(self.resize_value, self.resize_value), mode='bilinear', align_corners=True)
    return self.model(x)

  @staticmethod
  def add_model_specific_args(parent_parser):
    parser = parent_parser.add_argument_group('gcvit')
    parser.add_argument('--model_name',       type=str,   default='gc_vit_base')
    parser.add_argument('--multiply_filters', type=float, default=1.)
    parser.add_argument('--resize_value',     type=int,   default=224)
...
if __name__ == '__main__':
  model = gcvit(model_name='gc_vit_minimal_optolab',
    multiply_filters=1.,
    resize_value=128,
    loss_fn='mae',
    lr=1e-3,
