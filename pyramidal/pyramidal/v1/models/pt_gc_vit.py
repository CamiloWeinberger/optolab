...
from pyramidal.models.pt_Base import ModelBase
...
class GC_VIT_Lightning(ModelBase, pl.LightningModule):
  multiply_filters = 1.
  def __init__(self, model_name='gc_vit_base'):
    super().__init__()
    assert model_name in ['gc_vit_xxtiny', 'gc_vit_xtiny', 'gc_vit_tiny', 'gc_vit_small', 'gc_vit_base']
    self.model = create_model(model_name, pretrained=False, num_classes=199, multiply_filters=self.multiply_filters)
...
class GC_VIT_Lightning_xxtiny(GC_VIT_Lightning):
  def __init__(self):
    super().__init__('gc_vit_xxtiny')

class GC_VIT_Lightning_xxtiny_half(GC_VIT_Lightning):
  def __init__(self):
