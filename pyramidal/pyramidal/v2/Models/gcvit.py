...
from pyramidal.v2.Models._base import ModelBase
...
class gcvit(ModelBase, pl.LightningModule):
  multiply_filters = 1.
  use_class_args = False
  datapath_class = ''
  def __init__(self, model_name='gc_vit_base', datapath=''):
    super().__init__()
    if not self.use_class_args:
        if datapath == '':
            num_classes = 199
        else:
            num_classes = 500
    else:
        if self.datapath_class == '':
            num_classes = 199
        else:
            num_classes = 500
    assert model_name in ['gc_vit_xxtiny', 'gc_vit_xtiny', 'gc_vit_tiny', 'gc_vit_small', 'gc_vit_base']
    self.model = create_model(model_name, pretrained=False, num_classes=num_classes, multiply_filters=self.multiply_filters)
    self.loss_fn = nn.MSELoss()
    self.save_hyperparameters()

    self.metrics = {}
    self.metrics['mae']     = nn.L1Loss()
    self.metrics['rmse']    = lambda x, y: torch.sqrt(nn.MSELoss()(x, y))

  def forward(self, x):
    return self.model(x)


class GC_VIT_Lightning_xxtiny(gcvit):
  def __init__(self):
    super().__init__('gc_vit_xxtiny')

class GC_VIT_Lightning_xxtiny_half(gcvit):
  def __init__(self):
    self.multiply_filters = 0.5
    super().__init__('gc_vit_xxtiny')

if __name__ == '__main__':
  model = GC_VIT_Lightning_xxtiny_half()
